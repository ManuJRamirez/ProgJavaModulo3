 ...
 * Clase App (main).
 * 
 * En esta clase hemos creado un programa que interactuando con la consola, realiza el cálculo en días hasta el proximo cumpleaños.
 * Las funciones son:
 * 
 * <ul>
 * <li>Solicitar una fecha de nacimiento al usuario.</li>
 * <li>Vamos a validar que esa fecha introducida cumpla con ciertos parámetros. Si no lo cumple, se le volverá a pedir. </li>
 * <li>Calcula la edad actual del usuario.</li>
 * <li>Detectar si hoy es el cumpleaños del usuario y felicitarlo.</li>
 * <li>Calcula los días que faltan hasta el próximo cumpleaños ymuestra la fecha de cuando será día.
 * <li>Preguntar si desea seguir volver a introducir una fecha nueva o cerrar el programa</li> 
 * </ul>
 */
public class App {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			do {
				...
				if (fechaNacimientoIntroducida.matches("\\d{4}-\\d{2}-\\d{2}")) {
					try {
						...
						System.out.println("==>> Error en la fecha introducida. Vuelva a intentarlo. <<== \n");
					}
				} else {
					System.out.println("==>> Formato de fecha introducida no válido. Siga el patrón \"aaaa-mm-dd\" <<== \n");
				}
			...
			if (cumpleEsteAnio.isEqual(fechaHoy)) {
				...
				System.out.println("Te faltan " + diasHastaProximoCumpleanio + " días hasta tu próximo cumpleaños en "
						+ proximoCumpleanio + ".");
			} else {
			...
			do {
				...
				System.out.println("Respuesta no válida. Debe responder con \"Si\" o \"No\".");
			} while (true);
