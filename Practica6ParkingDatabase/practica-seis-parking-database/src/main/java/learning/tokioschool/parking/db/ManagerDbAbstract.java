...
public abstract class ManagerDbAbstract {
	...
	static final String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS Coches (Matricula VARCHAR (10) NOT NULL PRIMARY KEY, Marca VARCHAR (10) NOT NULL, Modelo VARCHAR(20) NOT NULL, HoraEntrada TIMESTAMP NOT NULL, HoraSalida TIMESTAMP)";
	...
	static final String UPDATE_ENTRADA = "UPDATE Coches SET HoraEntrada=?, HoraSalida=NULL WHERE Matricula=?";
	...
	public void crearTabla() throws ClassNotFoundException, SQLException {
		try (Connection connection = iniConexion(); Statement statement = connection.createStatement()) {
			// Execute a query
	...
	/**
	 * Actualiza el registro de entrada, en el caso en el que un coche que se haya marchado, vuelva de nuevo al parking.
	 *
	 ...
	 * @return the int
	 */
	public abstract int updateReentrada(final String matricula, final LocalDateTime horaEntrada);
